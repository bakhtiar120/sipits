...
class Atur_pmpi extends CI_Controller
{
    ...
    function __construct()
    {
        ...
        $this->load->model(array('admin_model', 'pmpi_model'));
    }

    public function index($temp1 = "", $temp2 = "")
    {

        $data['data'] = $this->pmpi_model->get_kp_all();
        $data['temp'] = $temp1 . " " . $temp2;

        $this->load->view('admin/atur_pmpi', $data);
    }

    public function detail($id = 0)
    {
        //print_r($id);
        $data = $this->pmpi_model->get_kp_by_id($id);

        //print_r($data[0]);
        $this->load->view('admin/detail_pmpi', $data[0]);
    }

    public function edit($id_kp = 0)
    {

        $data = $this->pmpi_model->get_kp_by_id($id_kp);

        $this->load->view('admin/edit_pmpi', $data[0]);
    }
    ...
    public function update_data($id_kp = 0)
    {
        ...
        $hasil = $this->pmpi_model->update_kp($id_kp, $data);
        ...
        redirect('atur_pmpi/index/' . $temp1 . '/' . $temp2);
    }

    public function update_status($id_kp = 0, $status = 0)
    {
        $hasil = $this->pmpi_model->update_status_kp($id_kp, $status);
    ...
    {
        $hasil = $this->pmpi_model->hapus_kp($id);
